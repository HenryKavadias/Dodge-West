...
public class PhysicsPickup : MonoBehaviour
{
    ...
    float DynamicForceToObject(float objMass, bool loaded = false)
    {
        ...
        if (objMass < 1)
        {
            result = (minThrowPower + GetComponent<Rigidbody>().velocity.magnitude) * objMass;
            return result;
        }
    ...
    void ThrowObject(bool current = false)
    {
        ...
        else
        {
            ...
            thrownObj.GetComponent<Rigidbody>().AddForce(pickupTarget.forward *
                DynamicForceToObject(thrownObj.GetComponent<Rigidbody>().mass, true), ForceMode.Impulse);

            //thrownObj.GetComponent<Rigidbody>().AddForce(pickupTarget.forward *
            //    (DynamicForceToObject(thrownObj.GetComponent<Rigidbody>().mass)
            //    + GetComponent<Rigidbody>().velocity.magnitude), ForceMode.Impulse);
