...
public class AltPickup : MonoBehaviour
{
    ...
    [Header("Transparent Bypass")]
    public List<string> bypassMaterialNames = new List<string>();
    ...
    void ModifyObjectMaterial(Transform parent)
    {
        foreach (Transform child in parent)
        {
            ...
            if (child.gameObject.GetComponent<MeshRenderer>() != null)
            {
                ...
                if (curObj.GetComponent<MeshRenderer>().materials.Length > 1)
                {
                    foreach (
                        Material material in
                        curObj.GetComponent<MeshRenderer>().materials)
                    {
                        bool skip = false;

                        foreach (string matName in bypassMaterialNames)
                        {
                            // check for OutlineMask and OutlineFill 
                            if (material.name.ToString().Contains(matName))
                            {
                                // check for OutlineMask and OutlineFill 
                                skip = true;
                            }
                ...
                {
                    materials.Add(curObj.GetComponent<MeshRenderer>().material);
                }
    ...
    void ResetObjectMaterial(Transform parent)
    {
        foreach (Transform child in parent)
        {
                ...
                objectMaterialCounter < objectMaterialCount)
            {
                ...
                if (curObj.GetComponent<MeshRenderer>().materials.Length > 1)
                {
                    for (int i = 0; i < curObj.GetComponent<MeshRenderer>().materials.Length; ++i)
                    {
                        Debug.Log("re-apply");
                        curObj.GetComponent<MeshRenderer>().materials[i] = objectMaterials[objectMaterialCounter];
